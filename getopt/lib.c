...
OPI_DEF(getopt_,
  ...
  for (int i = 0; i < n; l = opi_cdr(l), i++) {
    if (opi_car(l)->type != opi_str_type)
  ...
  while ((opt = getopt(n + 1, argv, OPI_STR(s)->str)) > 0) {
    opi_t kv = opi_cons(opi_str_from_char(opt),
          optarg ? opi_str_new(optarg) : opi_nil);
    acc = opi_cons(kv , acc);
  }
...
{
  opi_builder_def_const(bldr, "getopt", opi_fn_new(getopt_, 2));
  return 0;
}
